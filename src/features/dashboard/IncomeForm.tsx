...
export function IncomeForm({
  ...
  form: IncomeFormData;
  onChange: (form: IncomeFormData) => void;
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="bg-background border border-border rounded-lg p-6">
        <h2 className="text-xl font-semibold text-foreground mb-4">Add Income</h2>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              value={form.category}
              onChange={(e) => onChange({ ...form, category: e.target.value as IncomeCategory })}
              className="w-full px-3 py-2 border border-border rounded-lg bg-background text-foreground focus:outline-none focus:ring-1 focus:ring-accent focus:border-accent"
            >
              {categories.map((cat) => (
